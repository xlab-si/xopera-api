...
validation_st='{"service_template": "service.yaml", "inputs": {"some_input": "this is a value"}}'
...
docker-compose exec control curl --fail-with-body        api:8080/validate/csar -H "Content-Type: application/json" -d '{"service_template": "service.yaml", "inputs": {"some_input": "this is a value"}}'

docker-compose exec control curl --fail-with-body -XPOST api:8080/deploy -H "Content-Type: application/json" -d '{"service_template": "service.yaml", "inputs": {"some_input": "this is a value"}}'
wait_for_completion
